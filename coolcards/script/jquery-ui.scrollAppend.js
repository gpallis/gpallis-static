...
(function($){$.widget("ui.scrollAppend", {
	...
	_create:function() {
		...
		var cache = localStorage.getItem('scroll_'+self.param_key);
			...
			//self.page = localStorage.getItem('p_'+self.param_key); - commented out, this makes p not reset at refresh, wtf.
			//console.log("resuming from page: "+self.page+" at "+timestamp);
			$(this.options.appendTo).append(cache);
			//if(self.options.callback) self.options.callback.apply(); - commented out, didn't want callback on creation
		}
	...
	append: function() {
		...
		if(self.options.loadingImage) {
			$(self.options.appendTo).append("<div id='scroll_append_loading' class='scroll_append_loading' style='position:absolute;bottom:-30'><img src='"+self.options.loadingImage+"'></div>");
		}
